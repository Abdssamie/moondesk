...
**Content Definition:**
- If flagged in §4.1, first add an ad-hoc delegation step: "Ad-Hoc Delegation – <purpose>" (optional ref to .agent/workflows/Research_Delegation_Guide.md or .agent/workflows/Debug_Delegation_Guide.md), then continue
...
2. **If User skips Review:** Proceed directly to **Manager Bootstrap Creation**.
  - **CRITICAL:** You must generate the Bootstrap Prompt using the **EXACT TEMPLATE** defined in your initiation prompt .agent/workflows/Setup_Agent_Initiation_Prompt.md.
  - **Context Recovery:** If you cannot retrieve the template word-for-word from your context, you must **READ** the .agent/workflows/Setup_Agent_Initiation_Prompt.md file to refresh your memory before generating the prompt. Do not approximate the template.
