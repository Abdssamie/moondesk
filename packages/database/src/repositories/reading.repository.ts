...
import type { Reading, CreateReadingInput, ReadingQueryParams, AggregatedReading } from '@moondesk/domain';
import type { IReadingRepository } from '@moondesk/domain';
import { getDb } from '../client';
import { readings } from '../schema/index';
import { Parameter, Protocol, ReadingQuality, ReadingStats } from '@moondesk/domain';
...
export class ReadingRepository implements IReadingRepository {
    ...
    async getAggregated(params: ReadingQueryParams): Promise<AggregatedReading[]> {
        // TimescaleDB time_bucket aggregation would go here
        // For now, return empty array - requires raw SQL for time_bucket
        console.warn('getAggregated requires TimescaleDB time_bucket - not yet implemented');
        return [];
    }
    ...
    async getStats(organizationId: string): Promise<ReadingStats> {
        ...
        const qualityScore = total > 0 ? Math.round((goodCount / total) * 100) : 100;
