...
export class OrganizationRepository implements IOrganizationRepository {
    ...
    private mapToOrganization(record: typeof organizations.$inferSelect): Organization {
        return {
            id: record.id,
            name: record.name,
            slug: record.slug,
...
export class OrganizationMembershipRepository implements IOrganizationMembershipRepository {
    ...
    async getByOrganizationId(organizationId: string): Promise<OrganizationMembership[]> {
        const results = await this.db
            .select()
            .from(organizationMemberships)
    ...
    async getByUserAndOrg(userId: string, organizationId: string): Promise<OrganizationMembership | null> {
        const results = await this.db
            .select()
            .from(organizationMemberships)
            .where(
    ...
    private mapToMembership(record: typeof organizationMemberships.$inferSelect): OrganizationMembership {
        return {
