...
import type { Command, CreateCommandInput, UpdateCommandStatusInput, ICommandRepository } from '@moondesk/domain';
...
export class CommandRepository implements ICommandRepository {
    ...
    async getBySensorId(sensorId: number, organizationId: string): Promise<Command[]> {
        const results = await this.db
            .select()
            .from(commands)
            .where(and(eq(commands.sensorId, sensorId), eq(commands.organizationId, organizationId)))
            .orderBy(desc(commands.createdAt));
    ...
    async getById(id: number, organizationId: string): Promise<Command | null> {
        const results = await this.db
            .select()
            .from(commands)
            .where(and(eq(commands.id, id), eq(commands.organizationId, organizationId)))
            .limit(1);
    ...
    async getPending(organizationId: string): Promise<Command[]> {
        const results = await this.db
            .select()
            .from(commands)
            .where(and(eq(commands.organizationId, organizationId), eq(commands.status, CommandStatus.Pending)))
            .orderBy(desc(commands.createdAt));
    ...
    async updateStatus(id: number, organizationId: string, input: UpdateCommandStatusInput): Promise<Command | null> {
        ...
        } else if (input.status === CommandStatus.Completed || input.status === CommandStatus.Failed) {
