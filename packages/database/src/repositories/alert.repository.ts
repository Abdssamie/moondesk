...
import type { Alert, CreateAlertInput, AcknowledgeAlertInput, AlertQueryParams, IAlertRepository, AlertStats } from '@moondesk/domain';
...
export class AlertRepository implements IAlertRepository {
    ...
    async getStats(organizationId: string): Promise<AlertStats> {
        ...
        recentTrend.forEach(item => {
            ...
            if (item.severity === 'critical' || item.severity === 'emergency') {
                criticalDailyTotals[item.date] = (criticalDailyTotals[item.date] || 0) + item.count;
            }
        });
        ...
        const dailyTrend = calcTrend(
            lastDate ? dailyTotals[lastDate] : 0,
            prevDate ? dailyTotals[prevDate] : 0
        );
    ...
    async getUnacknowledged(organizationId: string): Promise<Alert[]> {
        const results = await this.db
            .select()
            .from(alerts)
            .where(and(eq(alerts.organizationId, organizationId), eq(alerts.acknowledged, false)))
            .orderBy(desc(alerts.timestamp));
    ...
    async acknowledge(id: number, organizationId: string, input: AcknowledgeAlertInput): Promise<Alert | null> {
        const results = await this.db
            .update(alerts)
            .set({
                acknowledged: true,
