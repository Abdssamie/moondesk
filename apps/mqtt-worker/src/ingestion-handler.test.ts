...
import { handleMessage, refreshAllSensorThresholds, clearThresholdCache } from './ingestion-handler';
...
describe('Ingestion Handler', () => {
    ...
    describe('handleMessage', () => {
        it('should call handleSingleReading for "readings" action', async () => {
            ...
            vi.mocked(parseTopic).mockReturnValue({ organizationId: 'test-org', sensorId: 1, action: 'readings' });
            vi.mocked(parseMessage).mockReturnValue({ value: 25 });
            vi.mocked(readingRepo.insert).mockResolvedValue({ id: 1, value: 25, timestamp: new Date() } as any);

            await handleMessage('org/test-org/sensor/1/readings', Buffer.from(''));

            expect(readingRepo.insert).toHaveBeenCalled();
            expect(broadcastToApi).toHaveBeenCalledWith('reading', expect.any(Object));
        });
    ...
    describe('refreshAllSensorThresholds', () => {
        it('should fetch all sensors and update the cache', async () => {
            ...
            vi.mocked(sensorRepo.getAll).mockResolvedValue(sensors as any);

            await refreshAllSensorThresholds();

            expect(sensorRepo.getAll).toHaveBeenCalled();
        });
    });
});
