...
import { connectToApi, broadcastToApi, disconnectFromApi } from './api-broadcaster';
...
describe('api-broadcaster', () => {
  ...
  describe('connectToApi', () => {
    it('should connect to the API and resolve on successful connection', async () => {
      socketMock.on.mockImplementation((event: string, callback: () => void) => {
        if (event === 'connect') {
          callback();
        }
      });
    ...
    it('should resolve even if the connection fails', async () => {
      socketMock.on.mockImplementation((event: string, callback: (error: Error) => void) => {
  ...
  describe('broadcastToApi', () => {
    it('should emit an event if the socket is connected', async () => {
      ...
      expect(socketMock.emit).toHaveBeenCalledWith('worker:reading', expect.any(Object));
    });
